...
export default function DMList({ onOpenConversation, activeConvoId }) {
  ...
  useEffect(() => {
    axios.get('/api/dm').then(({ data }) => setConversations(data)).catch(console.error);
  }, []);
  ...
  const openDM = async (userId) => {
    try {
      const { data } = await axios.post(`/api/dm/open/${userId}`);
      ...
      <div className="dm-conversations">
        ...
        {conversations.map(convo => {
          ...
          return (
            <button
              key={convo._id}
              className={`dm-item ${activeConvoId === convo._id ? 'active' : ''}`}
