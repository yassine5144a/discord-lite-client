...
export default function MembersList({ server, onServerUpdated }) {
  const { user } = useAuth();
  const [contextMenu, setContextMenu] = useState(null); // { memberId, x, y }
  ...
  const changeRole = async (memberId, role) => {
    try {
      const { data } = await axios.patch(`/api/servers/${server._id}/members/${memberId}/role`, { role });
  ...
  const kickMember = async (memberId) => {
    ...
    try {
      const { data } = await axios.delete(`/api/servers/${server._id}/members/${memberId}`);
      onServerUpdated?.(data);
    } catch (err) {
      alert(err.response?.data?.message || 'Error');
    }
  ...
  const MemberItem = ({ member }) => {
    ...
    return (
      <div
        ...
        onContextMenu={canManage ? (e) => { e.preventDefault(); setContextMenu({ memberId, x: e.clientX, y: e.clientY }); } : undefined}
      >
        ...
        {canManage && (
          <button
            ...
            onClick={(e) => { e.stopPropagation(); setContextMenu({ memberId, x: e.clientX, y: e.clientY }); }}
