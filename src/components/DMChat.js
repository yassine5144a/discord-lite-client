...
export default function DMChat({ conversation }) {
  ...
  const loadMessages = useCallback(async (p = 1) => {
    ...
    try {
      const { data } = await axios.get(`/api/dm/${conversation._id}/messages?page=${p}&limit=30`);
      if (p === 1) {
    ...
    <div className="dm-chat">
      ...
      <div className="dm-chat-header">
        <div className="avatar" style={{ width: 32, height: 32, fontSize: 11 }}>
          {otherUser?.avatar
            ? <img src={`${SERVER_URL}${otherUser.avatar}`} alt={otherUser?.username} />
      ...
      <div className="dm-messages">
        {hasMore && (
          <button className="load-more-btn" onClick={async () => { const next = page + 1; await loadMessages(next); setPage(next); }}>
            Load older messages
          </button>
        )}
        ...
        {grouped.map(item => {
          ...
          return (
            <div key={item.key} className={`dm-message ${showHeader ? 'with-header' : 'compact'} ${isOwn ? 'own' : ''}`}>
              {showHeader && (
              ...
              {!showHeader && <div style={{ width: 34, flexShrink: 0 }} />}
              <div className="dm-message-body">
                {showHeader && (
                  <div className="dm-message-header">
                    <span className="dm-message-author">{msg.author?.username}</span>
                    <span className="dm-message-time">{formatTime(msg.createdAt)}</span>
                  </div>
                )}
                ...
                {msg.attachment && (
                  <div className="attachment">
                    {msg.attachmentType === 'image'
                      ? <img src={`${SERVER_URL}${msg.attachment}`} alt="attachment" className="attachment-image" loading="lazy" onClick={() => window.open(`${SERVER_URL}${msg.attachment}`, '_blank')} />
